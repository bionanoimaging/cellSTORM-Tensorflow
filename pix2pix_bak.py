...
def main():
    ...
    C2Pmodel = model.create_model(examples.inputs, examples.targets, opt.ndf, opt.ngf, EPS, opt.gan_weight, opt.l1_weight, opt.l1_sparse_weight, opt.lr, opt.beta1)
#    C2Pmodel = model.create_model(examples.spikes, examples.targets, opt.ndf, opt.ngf, EPS, opt.gan_weight, opt.l1_weight, opt.l1_sparse_weight, opt.lr, opt.beta1)
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            "inputs": tf.image.encode_png(tf.expand_dims(tf.squeeze(converted_inputs), axis=2)),
            "targets": tf.image.encode_png(tf.expand_dims(tf.squeeze(converted_targets), axis=2)),
            "outputs": tf.image.encode_png(tf.expand_dims(tf.squeeze(converted_outputs), axis=2)), 
            "outputs_psf": tf.image.encode_png(tf.expand_dims(tf.squeeze(converted_outputs_psf), axis=2))
        }
        ...
        if opt.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if opt.is_video == True:
                    ...
                    inputs_np, outputs_np, outputs_psf_np = sess.run([inputs, outputs, outputs_psf], feed_dict= {inputs : input_frame})
                    ...
                    outputs_np = results["outputs"]
                    inputs_np = results["inputs"]
            ...
            for step in range(max_steps):
                ...
                if should(opt.progress_freq):
                    ...
                    fetches["gen_loss_sparse_L1"] = C2Pmodel.gen_loss_sparse_L1
                ...
                if should(opt.progress_freq):
                    ...
                    print("gen_loss_sparse_L1", results["gen_loss_sparse_L1"])
