...
def gen_deconv_npu_mod_2(batch_input, out_channels):
     ...
     with tf.variable_scope("deconv"):
        ...
        if(int(_c) <= 256):
            if(h>=128 and _c > 32):#(4*_b+ _b + _b) * (_c+15) > 1000): # taken from the HiAi Datasheet!
            
                print("Now performing weird deconv 3")
                # only produce patches in x/y not in channels! 

                # [batch, in_height, in_width, in_channels] => [batch, out_height, out_width, out_channels]
                #initializer = tf.random_normal_initializer(0, 0.02)
                
                print("Now performing weird deconv 2")
                # weird anyway - add a pseudo patch to the Tensor to make it work with NPU
                _c_mod = 256-int(_c)+16
                pseudo_patch = tf.slice(batch_input, [0,0,0,0], [int(_b), int(h), int(w), _c_mod])
                batch_input = tf.concat([batch_input, pseudo_patch], axis=-1)
                
                
                w_mod = 16 #int(int(_c)/4) # also take into account that stride only takes mod 16 strides! 
                h_mod = 16 #int(int(_c)/4) # also take into account that stride only takes mod 16 strides! 
                # image_resize conflicts with the dimensions, so trying to split and later concat the layers

                batch_input_sub = batch_input
                ...
                for w_i in range(0,int(int(w)/w_mod)):
                        batch_input_sub_patch_w = tf.slice(batch_input_sub, [0,0,w_i*w_mod,0], [int(_b), int(h), int(w_mod), _c_mod])

                        for h_i in range(0,int(int(h)/h_mod)):
                            batch_input_sub_patch_h = tf.slice(batch_input_sub_patch_w, [0,h_i*h_mod,0,0], [int(_b), int(h_mod), int(w_mod), _c_mod])
                            resized_input_sub_patch_h = tf.image.resize_images(batch_input_sub_patch_h, [int(h_mod) * 2, int(w_mod) * 2], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
                ...
                resized_input =  tf.slice(batch_input, [0,0,0,0], [int(_b), int(h) * 2, int(w) * 2, int(_c)])        
                ...
                _c_mod = 256-int(_c)+16
                print(_c_mod)
                pseudo_patch = tf.slice(batch_input, [0,0,0,0], [int(_b), int(h), int(w), int(_c_mod)],name='first_slice')
                batch_input = tf.concat([batch_input, pseudo_patch], axis=-1)
                batch_input = tf.image.resize_images(batch_input, [int(h) * 2, int(w) * 2], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
                resized_input = tf.slice(batch_input, [0,0,0,0], [int(_b), int(h) * 2, int(w) * 2, int(_c)],name='second_slice')

            
        else:
             resized_input = tf.image.resize_images(batch_input, [int(h) * 2, int(w) * 2], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def gen_deconv_npu_mod_3(batch_input, out_channels):
    # only produce patches in x/y not in channels! 
     ''' Transposed Convolution. '''
     with tf.variable_scope("deconv_3"):
        ...
        # now it's getting really weird, trying to further reduce the size of the images by patches 
        for w_i in range(0,int(int(w)/w_mod)):
            batch_input_sub_patch_w = tf.slice(batch_input, [0,0,w_i*w_mod,0], [_b, h, w_mod, _c])

            for h_i in range(0,int(int(h)/h_mod)):
                batch_input_sub_patch_h = tf.slice(batch_input_sub_patch_w, [0,h_i*h_mod,0,0], [_b, h_mod, w_mod, _c])
                # do resizing only on patches - possible because of 2x NN kernel
                resized_input_sub_patch_h = tf.image.resize_images(batch_input_sub_patch_h, [h_mod * 2, w_mod * 2], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        ...
        print(resized_input_sub)

        return tf.layers.separable_conv2d(resized_input_sub, out_channels, kernel_size=4, strides=(1, 1), padding="same", depthwise_initializer=initializer, pointwise_initializer=initializer)
...
def create_generator(generator_inputs, generator_outputs_channels, NGF, is_training=True):
    ...
    for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
        ...
        with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
            ...
            output = gen_deconv_npu_mod_2(rectified, out_channels)
    ...
    with tf.variable_scope("decoder_1"):
        ...
        rectified = tf.nn.relu(input)
        output = gen_deconv_npu_mod_2(rectified, generator_outputs_channels)
...
#        discrim_loss = tf.reduce_mean(predict_real + predict_fake)
    ...
    return Model(
        ...
        gen_loss_sparse_L1=gen_loss_sparse_L1,
