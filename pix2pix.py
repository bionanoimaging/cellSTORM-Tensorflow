...
# test 2:  --mode test --input_dir /home/diederich/Documents/STORM/DATASET_NN/All_Videos/2017-12-18_18.29.45.mp4 --batch_size 1 --output_dir ./dump --scale_size 1792 --checkpoint cellstorm_simple --roi_size 360 --is_csv 1 --is_tif 0 --is_frc 1 --x_center -1 --y_center -1 --max_steps 1000
...
inputs_tf = tf.placeholder(tf.float32, shape=(opt.batch_size, im_xdim, im_ydim, 1), name='inputs_tf')
outputs_tf = tf.placeholder(tf.float32, shape=(opt.batch_size, im_xdim, im_ydim, 1), name='outputs_tf')
spikes_tf = tf.placeholder(tf.float32, shape=(opt.batch_size, im_xdim, im_ydim, 1), name='spikes_tf')
...
if opt.mode == "test":
    ...
    out_path_inputs = os.path.join(myfile_dir, experiment_name+'output_sum_1.tif')    
    ...
    if opt.is_frc == 1: # if frc is true, there will be two sum-files to compute the FRC
        ...
        out_path_inputs_combined = os.path.join(myfile_dir, experiment_name+'output_sum_2_combined.tif')    
        imsave(out_path_inputs_2, np.float32(out_sum + out_sum_2)) # also supports 64bit but ImageJ does not    
        ...
        for step in range(max_steps-opt.batch_size-1):
            ...
            try:
                ...
                if should(opt.display_freq):
                    ...
                    summary_writer.add_summary(summary, step*(epoch_i+1))
